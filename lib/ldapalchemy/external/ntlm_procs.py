...
#
# Minor modifications by Cleber Rodrigues <cleber.gnu@gmail.com>
#
...
def ntlm_password_to_ldap_format(hash):
    """
    Return the LM/NT password in a format suitable for an LDAP directory
    """
    hash = hash[:16]
    return "".join(["%02X" % ord(i) for i in hash])

def ldap_nt_password(passwd):
    """
    Returns a NT password suitable for an LDAP directory
    """
    return ntlm_password_to_ldap_format(create_NT_hashed_password(passwd))

def ldap_lm_password(passwd):
    """
    Returns a LM password suitable for an LDAP directory
    """
    return ntlm_password_to_ldap_format(create_LM_hashed_password(passwd))
...
if __name__ == '__main__':
    ...
    print ldap_nt_password(password)
    print ldap_lm_password(password)
